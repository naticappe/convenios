// ============================================================
// PÁGINA: Nomenclador
// Módulo de gestión de nomencladores (tipos de prácticas médicas).
// Solapas: 1. Maestro  |  2. Auditoría
// ============================================================

import { useState, useRef } from 'react'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import { nomencladorMaestroService } from '../services/nomencladorMaestroService'
import type {
  NomencladorMaestroList, CrearEditarNomencladorMaestro,
  NomencladorMaestroAuditLog, EstadoNomencladorMaestro,
  NomencladorMaestroImportPreview, NomencladorMaestroImportPreviewRow
} from '../types'
import {
  Plus, Pencil, Search, X, Download, Upload,
  History, CheckCircle2, XCircle, Clock, Target,
  RefreshCw, AlertTriangle
} from 'lucide-react'

const ESTADOS: EstadoNomencladorMaestro[] = ['Vigente', 'Inactivo']
...
const FORM_INICIAL: CrearEditarNomencladorMaestro = {
  nombre: '', vigenciaDesde: HOY, vigenciaHasta: '9999-12-31',
}
...
export default function NomencladorMaestroPage() {
  ...
  const [editando, setEditando]     = useState<NomencladorMaestroList | null>(null)
  const [form, setForm]             = useState<CrearEditarNomencladorMaestro>(FORM_INICIAL)
  const [errorForm, setErrorForm]   = useState<string | null>(null)

  // Modal Historial
  const [modalHistorial, setModalHistorial] = useState(false)
  const [historialItem, setHistorialItem]   = useState<NomencladorMaestroList | null>(null)

  // Modal Importación
  const [modalImport, setModalImport]   = useState(false)
  const [preview, setPreview]           = useState<NomencladorMaestroImportPreview | null>(null)
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ['nomencladores-maestro', filtroBuscar, filtroEstado],
    queryFn:  () => nomencladorMaestroService.listar({
  ...
  const { data: historialData = [], isLoading: loadingHistorial } = useQuery({
    queryKey: ['nomenclador-historial', historialItem?.id],
    queryFn:  () => nomencladorMaestroService.auditoriaPorId(historialItem!.id),
    enabled:  !!historialItem,
  })

  const { data: auditoriaData = [], isLoading: loadingAuditoria } = useQuery({
    queryKey: ['nomenclador-auditoria', audUsuario, audAccion, audDesde, audHasta],
    queryFn:  () => nomencladorMaestroService.auditoriaGlobal({
  ...
  const guardar = useMutation({
    mutationFn: (datos: CrearEditarNomencladorMaestro) =>
      editando
        ? nomencladorMaestroService.actualizar(editando.id, datos)
        : nomencladorMaestroService.crear(datos),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['nomencladores-maestro'] }); cerrarModal() },
    onError:   (err: Error) => setErrorForm(err.message),
  })

  const toggleEstado = useMutation({
    mutationFn: ({ id, activo }: { id: number; activo: boolean }) =>
      nomencladorMaestroService.actualizarEstado(id, { activo }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['nomencladores-maestro'] }),
  })
  ...
  function abrirEditar(item: NomencladorMaestroList) {
    setEditando(item)
    setForm({ nombre: item.nombre, vigenciaDesde: item.vigenciaDesde, vigenciaHasta: item.vigenciaHasta })
  ...
  function abrirHistorial(item: NomencladorMaestroList) {
  ...
  async function handleExportar() {
    try {
      const blob = await nomencladorMaestroService.exportar()
      const url  = URL.createObjectURL(blob)
      const a    = document.createElement('a')
      a.href = url
      a.download = `nomencladores-${new Date().toISOString().slice(0, 10)}.xlsx`
      a.click()
  ...
  async function handleArchivoSeleccionado(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const data = await nomencladorMaestroService.importarPreview(archivo)
      setPreview(data)
    } catch (err: unknown) {
  ...
  async function handleConfirmarImport() {
    ...
    try {
      const res = await nomencladorMaestroService.importarConfirmar({
      ...
      setPreview(null)
      qc.invalidateQueries({ queryKey: ['nomencladores-maestro'] })
    } catch (err: unknown) {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <Target size={22} className="text-azul" />
            Nomencladores
          </h1>
          <p className="text-slate-500 text-sm mt-0.5">Gestión de tipos de nomencladores para prácticas médicas</p>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 mb-5">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm border border-azul-oscuro rounded-lg hover:bg-azul/5 text-azul-oscuro"
            >
            ...
            >
              <Plus size={16} />Nuevo Nomenclador
            </button>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 text-slate-400">
              ...
              <p>No se encontraron nomencladores.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200" style={{ backgroundColor: '#8AB1CB' }}>
                    ...
                    <th className="text-left px-4 py-3 font-semibold text-white">Vigencia Desde</th>
                    <th className="text-left px-4 py-3 font-semibold text-white">Vigencia Hasta</th>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-base font-semibold text-slate-800">
                {editando ? 'Editar Nomenclador' : 'Nuevo Nomenclador'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-1">
                ...
                <button type="submit" disabled={guardar.isPending}
                  ...
                  {guardar.isPending ? 'Guardando...' : (editando ? 'Guardar cambios' : 'Crear nomenclador')}
                </button>
              </div>
            </form>
          </div>
      ...
      {modalImport && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-base font-semibold text-slate-800">Importar Nomencladores</h2>
              ...
              {importOk ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-slate-500 text-sm">
                    Creados: <strong>{importOk.creadas}</strong> · Actualizados: <strong>{importOk.actualizadas}</strong>
                  </p>
                  <button onClick={cerrarImport}
                ...
                <>
                  <div className="mb-4">
                    <p className="text-sm text-slate-600 mb-3">
                      El archivo Excel debe tener: <strong>columna B = Nombre</strong>, <strong>C = Vigencia Desde</strong> (yyyy-MM-dd), <strong>D = Vigencia Hasta</strong>.
                  ...
                  {preview && (
                    <>
                      <div className="mb-3 text-sm text-slate-600">
                        ...
                        {' '}<span className="text-green-600">{preview.filas.filter(f => f.esNueva).length} nuevas</span>
                        {' · '}<span className="text-amber-600">{preview.filas.filter(f => !f.esNueva && (f.hayDiferenciaNombre || f.hayDiferenciaVigencia)).length} con cambios</span>
                        {' · '}<span className="text-slate-400">{preview.filas.filter(f => !f.esNueva && !f.hayDiferenciaNombre && !f.hayDiferenciaVigencia).length} sin cambios</span>
                      </div>
                      <div className="border border-slate-200 rounded-xl overflow-hidden mb-4">
                        <table className="w-full text-xs">
                          ...
                          <tbody>
                            {preview.filas.map((f: NomencladorMaestroImportPreviewRow, i: number) => (
                              <tr key={i} className="border-b border-slate-100">
                                ...
                                <td className="px-3 py-2 text-slate-400">
                                  {f.idExistente ? `${f.nombreActual} (${f.activoActual ? 'Vigente' : 'Inactivo'})` : '—'}
                                </td>
                                <td className="px-3 py-2">
                                  {f.esNueva ? (
                                    <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-full">NUEVA</span>
                                  ) : f.hayDiferenciaNombre || f.hayDiferenciaVigencia ? (
...
function AuditTable({ logs, compact = false }: { logs: NomencladorMaestroAuditLog[]; compact?: boolean }) {
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden">
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-slate-200" style={{ backgroundColor: '#8AB1CB' }}>
            {!compact && <th className="px-3 py-2.5 text-left font-semibold text-white">Nomenclador</th>}
            <th className="px-3 py-2.5 text-left font-semibold text-white">Acción</th>
            <th className="px-3 py-2.5 text-left font-semibold text-white">Fecha</th>
            <th className="px-3 py-2.5 text-left font-semibold text-white">Usuario</th>
            <th className="px-3 py-2.5 text-left font-semibold text-white">Origen</th>
            <th className="px-3 py-2.5 text-left font-semibold text-white">Detalle</th>
          </tr>
        </thead>
        <tbody>
          {logs.map(log => (
            <>
              <tr key={log.id} className="border-b border-slate-100 hover:bg-slate-50">
                {!compact && <td className="px-3 py-2 font-medium text-slate-700">{log.nombreNomenclador}</td>}
                <td className="px-3 py-2">
