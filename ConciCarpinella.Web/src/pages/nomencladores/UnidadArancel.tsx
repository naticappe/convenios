      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <Clock size={22} className="text-azul" />
            Unidad Arancel
          </h1>
          <p className="text-slate-500 text-sm mt-0.5">Gestión de unidades arancelarias</p>
        </div>
      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200" style={{ backgroundColor: '#8AB1CB' }}>
                    <th className="text-left px-4 py-3 text-white font-semibold w-12">ID</th>
                    <th className="text-left px-4 py-3 text-white font-semibold">Nombre</th>
                    <th className="text-left px-4 py-3 text-white font-semibold">Vigencia Desde</th>
                    <th className="text-left px-4 py-3 text-white font-semibold">Vigencia Hasta</th>
                    <th className="text-left px-4 py-3 text-white font-semibold">Estado</th>
                    <th className="text-right px-4 py-3 text-white font-semibold">Acciones</th>
                  </tr>
                </thead>
        ...
        <div className="flex flex-col gap-4 flex-1">
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
            {loadingAuditoria ? (
              ...
              <AuditTableUA logs={auditoriaData} />
            )}
          </div>
        </div>
      )}
      ...
      {modalHistorial && historialUnidad && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-base font-semibold text-slate-800">
                Historial — <span className="text-azul">{historialUnidad.nombre}</span>
              </h2>
            ...
            <div className="overflow-auto flex-1 p-4">
              {loadingHistorial ? (
                ...
                <AuditTableUA logs={historialData} compact />
              )}
            </div>
          </div>
        </div>
      ...
      {modalImport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-base font-semibold text-slate-800">Importar Unidades Arancel</h2>
...
// ── Tabla de auditoría (igual al estilo de ConceptoMaestro) ──
function AuditTableUA({ logs, compact = false }: { logs: UnidadArancelAuditLog[]; compact?: boolean }) {
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden">
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-slate-200" style={{ backgroundColor: '#8AB1CB' }}>
            {!compact && <th className="px-3 py-2.5 text-left font-semibold text-white">Unidad</th>}
          ...
          {logs.map(log => (
            <>
              <tr key={log.id} className="border-b border-slate-100 hover:bg-slate-50">
                {!compact && (
                  <td className="px-3 py-2 font-medium text-slate-700">{log.nombreUnidad}</td>
                )}
                <td className="px-3 py-2">
                  <span className={`px-2 py-0.5 rounded-full font-medium ${
                    ...
                    log.accion === 'CAMBIO_ESTADO'   ? 'bg-orange-100 text-orange-700' :
